...
const PlzIntegrations = () => {
    ...
    return (
        <section className="relative w-full bg-white py-24 font-sansation">
            <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">

                {/* Headers */}
                <div className="text-center mb-16 flex flex-col items-center">
                    <div className="px-4 py-1.5 border border-black/10 rounded-full text-xs text-gray-600 mb-6 inline-block tracking-widest uppercase bg-black/5">
                        Integraciones
                    </div>
                    <h2 className="text-3xl md:text-5xl font-medium text-gray-600 tracking-tight leading-tight max-w-4xl mb-6">
                        Pone los datos a tu servicio
                    </h2>
                    <p className="text-lg text-gray-500 font-light max-w-2xl leading-relaxed">
                        El agente de IA extrae datos de múltiples fuentes, los estandariza y limpia, apoyando la toma de decisiones con una intervención manual mínima.
                    </p>
                </div>

                {/* Two Column Layout */}
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-14 items-cente h-[520px]">
                    ...
                    <div className="flex flex-col gap-6 w-full">
                        <div className="w-full relative rounded-3xl overflow-hidden aspect-[4/3] md:aspect-[16/11] flex items-center justify-center p-6 shadow-2xl">
                            ...
                            <div className="relative z-10 bg-white/90 backdrop-blur-xl border border-black/10 rounded-2xl p-6 w-full max-w-sm shadow-2xl transform transition-all duration-500 hover:scale-105">
                                <p className="text-sm font-light text-gray-900 mb-6 leading-relaxed">
                        ...
                        <div className="flex justify-center items-center gap-2">
                            {integrations.map((_, idx) => (
                                <button
                                    ...
                                    className={`transition-all duration-300 border border-gray-300 ${activeIndex === idx ? 'w-8 h-2 bg-gray-900 rounded-full border-gray-900' : 'w-2.5 h-2.5 bg-gray-300 rounded-full hover:bg-gray-400 hover:border-gray-400'}`}
                    ...
                    <div className="flex flex-col w-full">
                        ...
                        {integrations.map((item, idx) => {
                            ...
                            return (
                                <div
                                    key={item.id}
                                    className={`border-b border-gray-200 cursor-pointer overflow-hidden transition-all duration-500`}
                                    onClick={() => setActiveIndex(idx)}
                                >
                                    <div className="py-4 flex flex-col justify-center">
                                        <h3 className={`text-2xl font-medium transition-colors duration-300 ${isActive ? 'text-gray-800' : 'text-gray-400 hover:text-gray-500'}`}>
                                            {item.title}
                                        </h3>
                                        <div
                                            className={`transition-all duration-500 ease-in-out ${isActive ? 'max-h-40 opacity-100 mt-3' : 'max-h-0 opacity-0'}`}
                                        >
                                            <p className="text-gray-500 font-light leading-relaxed pr-4">
                                                {item.desc}
                                            </p>
                                        </div>
                                    </div>
