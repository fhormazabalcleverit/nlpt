            ...
            <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                >
                    ...
                    {Object.entries(tabData).map(([key, data]) => (
                        <div
                            key={key}
                            className={`absolute inset-0 bg-cover bg-center bg-no-repeat transition-all duration-700 ${activeTab === key ? 'opacity-100' : 'opacity-0 scale-105 pointer-events-none'}`}
                    ...
                    <div className="relative z-10 w-full h-full p-8 md:p-16 flex flex-col justify-end">
                        <div className="max-w-2xl transition-all duration-500" key={`content-${activeTab}`}>
                            <h3 className="text-2xl md:text-3xl text-white font-medium mb-4 tracking-tight animate-fade-in-up">
                                {currentData.title}
                            </h3>
                            <p className="text-md md:text-lg text-gray-300 font-light mb-8 leading-relaxed animate-fade-in-up" style={{ animationDelay: '100ms' }}>
                                {currentData.desc}
                            </p>

                            {/* Tags/Badges Grid */}
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-10 max-w-xl animate-fade-in-up" style={{ animationDelay: '200ms' }}>
